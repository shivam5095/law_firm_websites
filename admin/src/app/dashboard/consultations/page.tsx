...
export default function ConsultationsPage() {
  ...
  // Selected consultation for viewing detail modal
  ...
  async function fetchConsultations() {
    try {
      ...
      console.error(err);
      setErrorMsg(err.message || 'Failed to fetch consultations list.');
    } finally {
      setLoading(false);
    }
  }
    ...
    try {
      ...
      if (response.success) {
        ...
        setConsultations((prev) =>
          prev.map((c) => (c.id === id ? { ...c, status: response.data.status } : c))
        );
        if (selectedCon && selectedCon.id === id) {
          setSelectedCon((prev) => prev ? { ...prev, status: response.data.status } : null);
        }
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="font-serif text-3xl text-navy-800 font-medium">Consultations</h1>
          <p className="text-charcoal-500 text-sm mt-1">Manage consultation requests and case inquiries.</p>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {consultations.map((con) => (
                  <tr key={con.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4">
                      <div>
                        <p className="font-semibold text-slate-900">{con.name}</p>
                        <p className="text-xs text-slate-500">{con.email} · {con.phone}</p>
                      </div>
                    </td>
                    <td className="p-4">
                      <div>
                        <p className="text-slate-700 font-medium">{con.matterType}</p>
                        <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                          {con.preferredMode === 'Phone' && <Phone className="w-3 h-3" />}
                          {con.preferredMode === 'Video' && <Video className="w-3 h-3" />}
                          {con.preferredMode === 'Office' && <Building className="w-3 h-3" />}
                          <span>{con.preferredMode}</span>
                        </p>
                      </div>
                    </td>
                    <td className="p-4 text-slate-600">
                      {new Date(con.preferredDate).toLocaleDateString('en-IN', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric'
                      })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white max-w-2xl w-full border border-slate-200 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-200 bg-navy-800 text-white flex items-center justify-between">
              <h2 className="font-serif text-xl font-medium">Consultation Details</h2>
              <button onClick={() => setSelectedCon(null)} className="text-white/80 hover:text-white">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-slate-700">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Client Name</span>
                  <p className="font-semibold text-navy-800 mt-1">{selectedCon.name}</p>
                </div>
                <div>
                  <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Subject Matter</span>
                  <p className="font-semibold text-gold-600 mt-1">{selectedCon.matterType}</p>
                </div>
                <div>
                  <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Email Address</span>
                  <p className="mt-1">{selectedCon.email}</p>
                </div>
                <div>
                  <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Phone Number</span>
                  <p className="mt-1">{selectedCon.phone}</p>
                </div>
                <div>
                  <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Preferred Slot</span>
                  <p className="mt-1 flex items-center gap-1.5">
                    <Calendar className="w-4 h-4 text-slate-500" />
                    <span>
                      {new Date(selectedCon.preferredDate).toLocaleDateString('en-IN', {
                        dateStyle: 'long'
                      })}
                    </span>
                  </p>
                ...
                <div>
                  <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Preferred Mode</span>
                  <p className="mt-1 flex items-center gap-1.5">
                    {selectedCon.preferredMode === 'Phone' && <Phone className="w-4 h-4 text-slate-500" />}
                    {selectedCon.preferredMode === 'Video' && <Video className="w-4 h-4 text-slate-500" />}
                    {selectedCon.preferredMode === 'Office' && <Building className="w-4 h-4 text-slate-500" />}
                    <span>{selectedCon.preferredMode}</span>
                  </p>
                </div>
              </div>

              <div>
                <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Brief Case Description</span>
                <div className="bg-slate-50 border border-slate-100 p-4 mt-2 text-sm whitespace-pre-wrap leading-relaxed">
                  {selectedCon.message || 'No description provided.'}
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {['NEW', 'CONTACTED', 'IN_PROGRESS', 'CLOSED'].map((status) => (
                    <button
                      ...
                      onClick={() => handleStatusChange(selectedCon.id, status)}
                      className={`px-4 py-2 text-xs font-semibold transition-colors rounded-none ${
                        selectedCon.status === status
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white max-w-md w-full border border-slate-200 p-6 shadow-2xl">
            ...
            <p className="text-slate-600 text-sm mb-6 leading-relaxed">
              Are you sure you want to permanently delete this consultation request? This action is irreversible.
            </p>
